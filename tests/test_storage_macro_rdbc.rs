...
async fn test_storage_macro_rdbc_mysql() -> Result<(), Box<dyn std::error::Error + Send + Sync + 'static>>
{
    ...
    let key_name = "storage_macro_test:db:$rdbc";
    cl.create_entry("storage_macro_test:db:url", b"mysql://" as &[u8])
        .await?;
    cl.create_entry("storage_macro_test:db:username", b"root" as &[u8])
        .await?;
    cl.create_entry("storage_macro_test:db:q0", b"SELECT * FROM users WHERE email = ?" as &[u8])
        .await?;
    cl.create_entry("storage_macro_test:db:p0", b"storage_macro" as &[u8])
        .await?;
