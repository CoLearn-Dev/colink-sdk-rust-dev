...
impl crate::application::CoLink {
    async fn _mm_create_entry(&self, key_name: &str, payload: &[u8]) -> Result<String, Error> {
    ...
    async fn _mm_read_entry(&self, key_name: &str) -> Result<Vec<u8>, Error> {
        let token = key_name.split("$").last().unwrap();
        if token.eq("chunk") {
    ...
    async fn _mm_update_entry(&self, key_name: &str, payload: &[u8]) -> Result<(), Error> {
        let token = key_name.split("$").last().unwrap();
        if token.eq("chunk") {
    ...
    async fn _mm_delete_entry(&self, key_name: &str) -> Result<(), Error> {
        let token = key_name.split("$").last().unwrap();
        if token.eq("chunk") {
